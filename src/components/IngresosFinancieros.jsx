...
import { Plus, Trash2, Edit2, Save, X, Wallet, Calendar, FileText, Link as LinkIcon, Download } from 'lucide-react';
...
function IngresosFinancieros({ user }) {
  ...
  const descargarPDF = () => {
    ...
    doc.text('Registro de Ingresos - Confirmación 2026', pageWidth / 2, 15, { align: 'center' });

    doc.setFontSize(10);
    doc.setFont('helvetica', 'normal');
    doc.text(`Total: ${totalIngresos.toLocaleString()} CRC | Generado: ${new Date().toLocaleDateString('es-CR')}`, pageWidth / 2, 22, { align: 'center' });

    const tableData = ingresos.map(i => [
      new Date(i.fecha).toLocaleDateString('es-CR'),
      i.origen,
      i.metodo.charAt(0).toUpperCase() + i.metodo.slice(1),
    ...
    autoTable(doc, {
      head: [['Fecha', 'Origen', 'Método', 'Monto']],
      body: tableData,
      startY: 28,
      theme: 'grid',
    ...
    doc.save('Ingresos_Confirmacion_2026.pdf');
  };
