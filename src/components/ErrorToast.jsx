...
const ErrorToast = () => {
  ...
  const getStyles = (type) => {
    const baseClasses = 'flex items-start gap-3 p-4 rounded-lg shadow-lg backdrop-blur-sm';
  ...
  return (
    <div className="fixed bottom-4 right-4 flex flex-col gap-3 z-50 max-w-md pointer-events-none">
      {errors.map((error) => (
        <div
          key={error.id}
          className={`${getStyles(error.type)} pointer-events-auto transition-all duration-300 animate-in fade-in slide-in-from-right`}
        >
          {getIcon(error.type)}
          <div className="flex-1">
            <p className="font-semibold text-sm">{error.message}</p>
            {error.data && process.env.NODE_ENV === 'development' && (
