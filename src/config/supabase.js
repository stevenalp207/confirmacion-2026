import { createClient } from '@supabase/supabase-js';
import { registerSupabaseMutationAudit } from '../utils/auditLogs';

const SUPABASE_URL = 'https://zuoaywpcqwsnjqdmdwms.supabase.co';
const SUPABASE_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Inp1b2F5d3BjcXdzbmpxZG1kd21zIiwicm9sZSI6ImFub24iLCJpYXQiOjE3Njg1MTk3NDcsImV4cCI6MjA4NDA5NTc0N30.FqLEJQCwGmrzUuImBdam42R2tNj3TD3OldFpmud95q4';

const auditedFetch = async (input, init) => {
  const startedAt = Date.now();
  const response = await fetch(input, init);

  try {
    registerSupabaseMutationAudit({
      input,
      init,
      response,
      durationMs: Date.now() - startedAt,
      supabaseUrl: SUPABASE_URL,
      supabaseAnonKey: SUPABASE_ANON_KEY
    });
  } catch {
    // La auditoria no debe bloquear la app.
  }

  return response;
};

export const supabase = createClient(SUPABASE_URL, SUPABASE_ANON_KEY, {
  global: {
    fetch: auditedFetch,
    headers: {
      'X-Client-Info': 'confirmacion-2026'
    }
  },
  db: {
    schema: 'public'
  },
  auth: {
    persistSession: true,
    autoRefreshToken: true
  }
});
